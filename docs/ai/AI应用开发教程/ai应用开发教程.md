...
# 39 认识大模型 Agent
大模型应用需要 agent 的原因：
- 大模型的幻觉问题，很难从模型本身解决，严肃场景需要引入外部知识确保答案的准确
- 大模型参数无法实时更新。多数场景难以满足实际需求
- 复杂业务场景，不是一问一答就能解决的，需要任务拆解、多步执行与交互

大模型的 Agent 技术框架：
- prompt llm answer
- RAG: prompt llm context
- Agent: memory tools  planing action

为什么需要做 Agent tuning 。通过 prompt engineering 可以实现 agent， 为什么还要训练 Agent 模型呢？

Agent Tuning 的研发流程：
Gpt-4+Prompt Engineering 调通流程 -> 构建训练数据 -> Agent Tuning 微调大模型替换GPT-4

# 40 Agent Prompt 模板设计

主流Agent Prompt 模板：
- ReAct： 先设定  Question, 再以 Thought、Action、Action Input、Observation 执行多轮，最后输出 Final Answer
- ModelScope 更为直接的生成回复，调用只需要加入`<|startofthinkg|>和|endofthin|>A` 字段，并在其填入 command_name和args
- AutoGPT: prompt 较为复杂。分为生成工具调用和生成最终答案两套prompt
- ToolLaMA : 模仿AutoGPT 和 ReAct

Agent Prompt1 组成：
- Profile 角色。定义 Agent 的身份、背景甚至性格特点等。帮助模型在回答时保持一致性和个性化
- Instruction 指令。提供具体操作指令，指导 agent 如何完成任务。明确告诉 Agent 特定环境下应该做什么或者避免做什么
- Tools 。 列出 agent 可以使用的工具或者功能。需要根据实际业务需求来构建
- Goal(目标，用户的 query)。帮助 Agent 聚焦于最终目标，如帮助用户解答问题、撰写文章等
- Memory(记忆)。 保持上下文或者回话记忆，提供连续性。包括已完成任务、对话历史、知识。
- Format(输出格式)。
- 其他必要信息。比如当前时间、地点、ip 等。

# 41 Agent Tuning (模型微调)

大模型 fine-tuning 的数据形式：本质上给定输入 input 的条件下，调整模型的参数，是的大模型输出和标准输出尽可能接近。
input(Prompt) output (response)

# 42 Agent 泛化性提升
提升训练数据的多样性。 Query、Tools、Agent Prompt 模板，最终的数据就是这3 个变量的组合。
...
Tools: 两种维度 Tools 描述格式多样性和Tools类型多样性
- Tools 描述格式多样性
- Tools类型多样性

Agent Prompt: 提升模板多样性

Meta-Agent Prompt: 用于调用gpt-4生成agent prompt 模板的 prompt， 称之为 Meta-Agent Prompt
...
# 44 Agent 概念、组成与决策
AI Agents 是基于 LLM 能够自主理解、自主决策规划、执行复杂任务的智能体。
...
# 46 Agent规划子任务详解 COT 与 TOT

规划(Planning)

子任务分解
  - 思维链COT
  - 思维树TOT。思维链的每一步，推理出多个分支

反思改进


# 47 思维树24点拓展和ReAct 框架
...
生活中的记忆机制：
- 感觉记忆。感官印象，只持续几秒钟
- 短期记忆。暂存和处理有限的信息。例如一个电话号码直到拨打完毕
- 长期记忆。可以存储大量信息。分为显性和隐性记忆

智能体：
- 形成记忆。预训练中，大模型通过调整神经元的权重来学习和理解人类语言
- 短期记忆。当前执行任务过程中所产生的信息。任务结束后清空
- 长期记忆。长期保留的信息。一般是外部数据库，通常用向量数据库存储和检索
...
json 工具包: langchain.community.tools.json.tool import JsonSpec
