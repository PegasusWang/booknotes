...
import (
	"sync"
	"test/source_code_read/hystrix-go/hystrix/rolling"
	"time"

	metricCollector "test/source_code_read/hystrix-go/hystrix/metric_collector"
)
...
type commandExecution struct {
	...
	ConcurrencyInUse float64       `json:"concurrency_in_use"`
}
...
func (m *metricExchange) Reset() {
	...
	for _, c := range m.metricCollectors {
		c.Reset()
	}
}
...
func (m *metricExchange) IsHealthy(now time.Time) bool {
	return m.ErrorPercent(now) < getSettings(m.Name).ErrorPercentThreshold // default 50
}
